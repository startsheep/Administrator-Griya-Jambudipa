                ...
                <div class="box d-flex justify-content-start mb-2">
                  <div class="row">
                    ...
                    <div class="ml-5">
                      ...
                      <strong v-if="house">{{
                        formatRupiah(countDiscount())
                      }}</strong>
                    </div>
                    <div class="ml-5">
                      <strong> Total : </strong>
                      <strong v-if="house">{{
                        formatRupiah(countPayment())
                      }}</strong>
                    </div>
                  </div>
                  <!-- <div class="column">
...
export default {
  ...
  computed: {
    ...
    formData() {
      ...
      if (this.payment.developPrice != null) {
        formData.append(
          "develop_price",
          Utils.currencyToNumber(this.payment.developPrice)
        );
      }
  ...
  methods: {
    ...
    countPayment() {
      let result =
        Utils.currencyToNumber(this.house.price ?? "Rp. 0") +
        Utils.currencyToNumber(this.payment.developPrice ?? "Rp. 0") -
        (this.house.price * this.payment.discount) / 100;
    ...
    createPayment() {
      ...
      if (
        this.payment.developPrice != null &&
        this.payment.developName === null
      ) {
    ...
    selectCustomer(val) {
      console.log("🚀 ~ file: FormPayment.vue:441 ~ selectCustomer ~ val", val);
      this.selectedCustomer = val;
      this.getKavling(val.id);
    },
    showNameCustomer(id) {
      let name = "";
    ...
    getHouse() {
      this.$store
        .dispatch("showData", [
          "house-type/" + this.selectedKavling.houseType.id,
          [],
        ])
        .then((res) => {
          this.house = res.data;
    ...
    getCustomers(event) {
      console.log(
        "🚀 ~ file: FormPayment.vue:469 ~ getCustomers ~ event",
        event
      );

      const self = this;
      const params = [`name=${event}`].join("&");
      self.$store.dispatch("getData", ["customer", params]).then((res) => {
    ...
    getKavling(id) {
      const self = this;
      self.$store
        .dispatch("showData", ["kavling/check/" + id, []])
        .then((res) => {
